...
EVALUATE
VAR FilteredTable = FILTER('Sales', 'Sales'[Quantity] > 0)
VAR HourlyTable = SELECTCOLUMNS(FilteredTable
, "SalesHour", HOUR('Sales'[InvoiceDate])
...
GROUPBY(
    HourlyTable,
    [SalesHour],
    "Total Sales", COUNTX(CURRENTGROUP(), [InvoiceNo]),
    "Total Revenue", SUMX(CURRENTGROUP(), [Revenue])
)
...
VAR SummaryTable = 
    SUMMARIZE(
        ...
        "Toplam Ciro", ROUND(SUMX('Sales', 'Sales'[Quantity] * 'Sales'[UnitPrice]), 2),
        "Sipariş Sayısı", DISTINCTCOUNT('Sales'[InvoiceNo])
    )
RETURN
    SummaryTable
...
VAR CustomerSummary = 
    SUMMARIZE(
        FILTER('Sales', NOT(ISBLANK('Sales'[Number of Customers])) && 'Sales'[Quantity] > 0),
        'Sales'[Number of Customers],
        "Total Visits", DISTINCTCOUNT('Sales'[InvoiceNo]),
        "Total Quantity Purchased", SUM('Sales'[Quantity]),
        "Total Revenue Generated", ROUND(SUMX('Sales', 'Sales'[Quantity] * 'Sales'[UnitPrice]), 2)
    )
RETURN
    TOPN(10, CustomerSummary, [Total Revenue Generated])
